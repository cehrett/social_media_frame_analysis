...
def load_data(cluster_label_loc, flags_loc, author_id_loc):
...
def save_results(df_suspicion, output_loc):
    df_suspicion.to_csv(output_loc, index=False, quoting=csv.QUOTE_ALL)
...
def run_analysis(args):
    ...
    df_cluster_labels, df_flags_for_each_author, df_author_id_for_each_post = load_data(args.cluster_label_loc, args.flags_loc, args.author_id_loc)
    df_model = preprocess_data(df_author_id_for_each_post, df_cluster_labels, args.flags)
    ...
    save_results(top_cluster_labels, args.output_loc)
...
default_output_loc = os.path.join('/', 'home', 'cehrett', 'Projects', 'Trolls', 'narr_extraction', 'user_facing_repo', 'top_suspicious_clusters.csv')
...
def parse_args():
    ...
    parser.add_argument('--output_loc', default=default_output_loc, help='Output path for results')
    parser.add_argument('--flags', default=default_flags, help='Flags of suspicious accounts included in data.')
